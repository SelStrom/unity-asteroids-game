    ...
    public class Game
    {
        ...
        private void ClearEcsEventBuffers()
        {
            ...
            if (rocketQuery.CalculateEntityCount() > 0)
            {
                ...
                for (int i = 0; i < entities.Length; i++)
                {
                    _entityManager.GetBuffer<RocketLaunchEvent>(entities[i]).Clear();
                }
        ...
        private void OnRocket()
        {
            if (TryGetShipEntity(out var entity))
            {
                if (!_entityManager.HasComponent<RocketLauncherData>(entity))
                {
                    return;
                }

                var launcher = _entityManager.GetComponentData<RocketLauncherData>(entity);
                var rotateData = _entityManager.GetComponentData<RotateData>(entity);
                var moveData = _entityManager.GetComponentData<MoveData>(entity);

                launcher.Launching = true;
                launcher.LaunchPosition = moveData.Position;
                launcher.LaunchDirection = rotateData.Rotation;
